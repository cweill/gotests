...
func BenchmarkBar_Foo7(b *testing.B) {
	type args struct {
		i int
	}
	benchmarks := []struct {
	...
	for _, bb := range benchmarks {
		...
		if (err != nil) != bb.wantErr {
			b.Errorf("%q. Bar.Foo7(%v) error = %v, wantErr %v", tt.name, tt.args.i, err, bb.wantErr)
			continue
		}
		if got != bb.want {
			b.Errorf("%q. Bar.Foo7(%v) = %v, want %v", tt.name, tt.args.i, got, bb.want)
		}
	}
}
