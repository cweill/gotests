...
func Benchmark_wrapToString(b *testing.B) {
	type args struct {
		in []int
	}
	benchmarks := []struct {
	...
	for _, bb := range benchmarks {
		b.Run(bb.name, func(t *testing.B) {
			if got := wrapToString(tt.args.in); !reflect.DeepEqual(got, bb.want) {
				b.Errorf("wrapToString() = %v, want %v", got, bb.want)
			}
		})
	}
}
